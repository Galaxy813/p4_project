...
try {
    ...
    // Stap 1: Verwijder eerst feedback gerelateerd aan meldingen van deze medewerker
    $db->query('DELETE FROM feedback WHERE MeldingId IN (
        SELECT Id FROM melding WHERE GebruikerId = :id
    )', [
        'id' => $id
    ]);

    // Stap 2: Verwijder meldingen van deze medewerker
    $db->query('DELETE FROM melding WHERE GebruikerId = :id', [
        'id' => $id
    ]);

    // Stap 3: Verwijder vervolgens de medewerker
