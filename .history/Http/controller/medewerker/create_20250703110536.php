...
if (!Validator::string($first, 4, 50)) {
    $errors['first'] = 'Voornaam moet minimaal 4 en maximaal 50 tekens lang zijn.';
}


if (!Validator::string($last, 4, 50)) {
    $errors['last'] = 'Achternaam moet minimaal 4 en maximaal 50 tekens lang zijn.';
}
...
try 
    ...
    // Haal het hoogste Nummer op voor de nieuwe medewerker
    $stmt = $db->query("SELECT MAX(Nummer) AS max FROM medewerker");
