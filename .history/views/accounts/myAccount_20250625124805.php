  ...
  <div class="testimonials__grid">
    <?php foreach ($mynotes as $Mynote): ?>
      <div class="card">
        ...
        <!-- Feedback per melding hieronder -->
        <?php if (!empty($feedbackPerMelding[$Mynote['MeldingId']] ?? [])): ?>
          <div class="mt-4 pl-4 border-l-4 border-blue-300 space-y-2">
            <?php foreach ($feedbackPerMelding[$Mynote['MeldingId']] as $feedback): ?>
              <div class="bg-blue-50 p-2 rounded">
                <p class="text-sm"><?= htmlspecialchars($feedback['FeedbackBericht']) ?></p>
                <p class="text-xs text-gray-500"><em>Door: <?= htmlspecialchars($feedback['VolledigeNaam']) ?> op <?= date('d-m-Y', strtotime($feedback['FeedbackDatum'])) ?></em></p>
              </div>
            <?php endforeach; ?>
          </div>
        <?php else: ?>
